...
/**
 * Convierte un GeoJSON de PUNTOS de UTM a WGS84.
 */
export function convertirGeoJsonUtmAWgs84(geojson: any): any {
  ...
  return { type: 'FeatureCollection', features: featuresCorregidas };
}

/**
 * Comprueba si una coordenada [lng, lat] se encuentra dentro de un polígono.
 */
...
/**
 * Calcula el Bounding Box [[Oeste, Sur], [Este, Norte]] de un anillo WGS84.
 */
export function obtenerBoundsDePoligono(poligono: [number, number][][]): [[number, number], [number, number]] {
  ...
  for (const [lng, lat] of anillo) {
    ...
    if (lng > maxLng) maxLng = lng; // ← bug corregido: era maxLng = maxLng = lng
